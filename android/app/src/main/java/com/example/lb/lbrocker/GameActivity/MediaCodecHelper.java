...
public class MediaCodecHelper {
    ...
    private static String getAdrenoVersionString(String glRenderer) {
        ...
        logInfo("Found Adreno GPU: "+modelNumber);
        return modelNumber;
    }
    ...
    public static void initialize(Context context, String glRenderer) {
        ...
        if (configInfo.reqGlEsVersion != ConfigurationInfo.GL_ES_VERSION_UNDEFINED) {
            logInfo("OpenGL ES version: "+configInfo.reqGlEsVersion);
            ...
            if (configInfo.reqGlEsVersion >= 0x30000) {
                logInfo("Added omx.nvidia to AVC reference frame invalidation support list");
                refFrameInvalidationAvcPrefixes.add("omx.nvidia");

                logInfo("Added omx.qcom/c2.qti to AVC reference frame invalidation support list");
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    logInfo("Added omx.intel to AVC reference frame invalidation support list");
            ...
            if (isGLES31SnapdragonRenderer(glRenderer)) {
                ...
                // HEVC anyway.
                logInfo("Added omx.qcom/c2.qti to deprioritized HEVC decoders based on GLES 3.1+ support");
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && isPowerVR(glRenderer)) {
                logInfo("Added omx.mtk to HEVC decoders based on PowerVR GPU");
                whitelistedHevcDecoders.add("omx.mtk");
                ...
                if (glRenderer.contains("GX6")) {
                    logInfo("Added omx.mtk to RFI list for HEVC");
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            try {
                if (decoderInfo.getCapabilitiesForType(mimeType).isFeatureSupported(FEATURE_LowLatency)) {
                    logInfo("Low latency decoding mode supported (FEATURE_LowLatency)");
                    return true;
                }
            } catch (Exception e) {
    ...
    public static boolean decoderSupportsAdaptivePlayback(MediaCodecInfo decoderInfo, String mimeType) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isDecoderInList(blacklistedAdaptivePlaybackPrefixes, decoderInfo.getName())) {
                logInfo("Decoder blacklisted for adaptive playback");
                return false;
            }
            ...
            try {
                        ...
                        isFeatureSupported(CodecCapabilities.FEATURE_AdaptivePlayback))
                {
                    ...
                    logInfo("Adaptive playback supported (FEATURE_AdaptivePlayback)");
                    return true;
                }
            } catch (Exception e) {
                // Tolerate buggy codecs
    ...
    public static boolean decoderIsWhitelistedForHevc(String decoderName, boolean meteredData) {
        ...
        if (isDecoderInList(deprioritizedHevcDecoders, decoderName)) {
            if (meteredData) {
                logInfo("Selected deprioritized decoder");
                return true;
            }
            else {
                return false;
        ...
        for (String preferredDecoder : preferredDecoders) {
            for (MediaCodecInfo codecInfo : getMediaCodecList()) {
                ...
                if (preferredDecoder.equalsIgnoreCase(codecInfo.getName())) {
                    logInfo("Preferred decoder choice is "+codecInfo.getName());
                    return codecInfo;
                }
            }
        }
    ...
    private static boolean isCodecBlacklisted(MediaCodecInfo codecInfo) {
        ...
        if (isDecoderInList(blacklistedDecoderPrefixes, codecInfo.getName())) {
            logInfo("Skipping blacklisted decoder: "+codecInfo.getName());
            return true;
        }
        ...
        for (MediaCodecInfo codecInfo : getMediaCodecList()) {
            ...
            for (String mime : codecInfo.getSupportedTypes()) {
                if (mime.equalsIgnoreCase(mimeType)) {
                    ...
                    logInfo("First decoder choice is "+codecInfo.getName());
                    return codecInfo;
                }
            }
        }
    ...
    private static MediaCodecInfo findKnownSafeDecoder(String mimeType, int requiredProfile) throws Exception {
        for (MediaCodecInfo codecInfo : getMediaCodecList()) {      
            ...
            for (String mime : codecInfo.getSupportedTypes()) {
                if (mime.equalsIgnoreCase(mimeType)) {
                    logInfo("Examining decoder capabilities of "+codecInfo.getName());
                    ...
                    if (requiredProfile != -1) {
                        for (CodecProfileLevel profile : caps.profileLevels) {
                            if (profile.profile == requiredProfile) {
                                logInfo("Decoder " + codecInfo.getName() + " supports required profile");
                                return codecInfo;
                            }
                        }

                        logInfo("Decoder " + codecInfo.getName() + " does NOT support required profile");
                    }
                    else {
                        return codecInfo;
                    }
            ...
            if (files != null) {
                for (File f : files) {
                    if (stringContainsIgnoreCase(f.getName(), "exynos4")) {
                        logInfo("Found exynos4 in /sys/devices/system");
                        return true;
                    }
                }
            }
